...
export default function InventoryDashboard() {
  ...
  useEffect(() => {
    setIngredients([
      ...
      { id: 4, name: "Gạo Tám Thơm", unit: "Kg", quantity: 495.0, price: 20000 },
      { id: 15, name: "Đường", unit: "Kg", quantity: 5.0, price: 18000 },
    ]);
    setSuppliers([
      { id: 1, name: "Nông Trại B", phone: "0909123456", email: "b@farm.com", address: "Dong Nai" }, // Thêm field mẫu
    ...
    <DashboardLayout>
      ...
      <div style={styles.header}>
        ...
        <div style={{display: 'flex', gap: '10px'}}>
          <button style={{...styles.btn, background: '#ff9800'}} onClick={() => openMainModal('audit')}>
            Tạo Phiếu Kiểm Kê
          </button>
          <button style={{...styles.btn, background: '#4caf50'}} onClick={() => openMainModal('import')}>
            Tạo Phiếu Nhập Kho
          </button>
        </div>
      </div>
        ...
        <div style={styles.overlay}>
          <div style={styles.modalLarge}>
            <div style={styles.modalHeader}>
              <h3>{modalType === 'import' ? '📥 Phiếu Nhập Hàng Mới' : '📋 Phiếu Kiểm Kê Kho'}</h3>
              <button onClick={closeMainModal} style={styles.closeBtn}>✕</button>
            </div>
            ...
            <div style={styles.modalBody}>
              <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '20px'}}>
                <div>
                  ...
                  <input disabled value={new Date().toLocaleString()} style={styles.inputDisabled} />
                </div>
              </div>
              ...
              <div style={styles.itemsSection}>
                ...
                <div style={styles.itemsHeader}>
                  <h4 style={{margin: 0, fontSize: '16px'}}>Danh sách chi tiết</h4>
                  <button style={styles.addBtn} onClick={openItemModal}>
                    {/* Kiểm tra modalType để hiển thị chữ tương ứng */}
                    {modalType === 'import' ? '+ Thêm nguyên liệu' : '+ Chọn nguyên liệu'}
                  </button>
                </div>
                      ...
                      <tr key={idx}>
                        ...
                        {modalType === 'import' ? (
                          <>
                            ...
                            <td style={{...styles.tdCenter}}>{row.quantity}</td>
                            <td style={styles.td}>{fmtMoney(row.price)}</td>
                            <td style={styles.td}>{fmtMoney(row.price * row.quantity)}</td>
                          </>
                        ) : (
                          <>
                            <td style={styles.tdCenter}>{row.current_sys}</td>
                            <td style={{...styles.tdCenter}}>{row.quantity}</td>
                            <td style={{...styles.tdCenter, color: (row.quantity - row.current_sys) < 0 ? 'red' : 'green'}}>
                        ...
                        <td style={styles.tdCenter}>
                          <button 
                            ...
                            style={{...styles.iconBtn, opacity: 0.7, fontSize: '16px'}}
                            title="Xóa dòng này"
                          >
                            🗑
                          </button>
            ...
            <div style={styles.modalFooter}>
              <button style={styles.submitBtn} onClick={submitMainForm}>Lưu</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div style={styles.modalSmall}>
            ...
            <div style={{display: 'flex', gap: '10px'}}>
              <div style={{flex: 1}}>
                ...
                <input disabled value={itemForm.unit} style={styles.inputDisabled} />
              </div>
              {modalType === 'audit' && (
                <div style={{flex: 1}}>
                  <label style={styles.label}>Tồn HT:</label>
                  <input disabled value={itemForm.current_sys} style={styles.inputDisabled} />
                </div>
              )}
            </div>
            ...
            <input 
              type="number" style={styles.input} autoFocus
            ...
            <div style={{display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '15px'}}>
              <button onClick={() => setShowItemModal(false)} style={styles.cancelBtn}>Hủy</button>
              <button onClick={saveItemRow} style={styles.submitBtnSmall}>Thêm</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{...styles.overlay, zIndex: 1200}}>
          <div style={styles.modalSmall}>
            ...
            <div style={{display: 'flex', justifyContent: 'flex-end', gap: '10px'}}>
              <button onClick={() => setShowQuickAddSupplier(false)} style={styles.cancelBtn}>Hủy</button>
              <button onClick={handleCreateSupplier} style={styles.submitBtnSmall}>Lưu</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showQuickAddIngredient && (
        <div style={{...styles.overlay, zIndex: 1200}}>
          <div style={styles.modalSmall}>
            <h4 style={{marginTop: 0, color: '#ff9800', marginBottom: '20px'}}>✨ Thêm Nguyên Liệu Mới</h4>
            ...
            <div style={{display: 'flex', gap: '10px'}}>
              <div style={{flex: 1}}>
                ...
                <input style={styles.input} placeholder="Kg, Lít..."
              ...
              <div style={{flex: 1}}>
                ...
                <input type="number" style={styles.input} 
                  value={newIngredient.price} onChange={e => setNewIngredient({...newIngredient, price: e.target.value})}
                />
              </div>
            </div>

            <div style={{display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '10px'}}>
              <button onClick={() => setShowQuickAddIngredient(false)} style={styles.cancelBtn}>Hủy</button>
              <button onClick={handleCreateIngredient} style={styles.submitBtnSmall}>Lưu Món</button>
            </div>
          </div>
        </div>
      )}
...
const styles = {
  ...
  btn: { padding: '10px 20px', border: 'none', borderRadius: '6px', color: 'white', fontWeight: 'bold', cursor: 'pointer' },
  ...
  thCenter: { textAlign: 'center', padding: '12px 15px', borderBottom: '2px solid #eee', color: '#444', fontWeight: 'bold', backgroundColor: '#f5f5f5' },
  tdCenter: { padding: '12px 15px', borderBottom: '1px solid #f0f0f0', textAlign: 'center' },
  ...
  modalLarge: { background: "white", borderRadius: "10px", width: "800px", maxWidth: "95%", height: "90vh", display: 'flex', flexDirection: 'column' },
  modalSmall: { background: "white", borderRadius: "10px", width: "350px", padding: '20px', boxShadow: "0 10px 30px rgba(0,0,0,0.3)" },
  ...
  closeBtn: { background: 'none', border: 'none', color: '#00003F', fontSize: '18px', cursor: 'pointer' },
  
  label: { display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '13px', color: '#555' },
  input: { width: '100%', padding: '8px', borderRadius: '4px', border: '1px solid #ccc', marginBottom: '10px' },
  ...
  addSmallBtn: { background: '#4caf50', color: 'white', border: 'none', width: '40px', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', fontSize: '20px', display: 'flex', justifyContent: 'center', alignItems: 'center', padding: 0, height: 'auto' },
  iconBtn: { background: 'none', border: 'none', cursor: 'pointer' },
  
  submitBtn: { padding: '10px 25px', background: '#5a381e', color: 'white', border: 'none', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer' },
  submitBtnSmall: { padding: '8px 16px', background: '#4caf50', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' },
  cancelBtn: { padding: '8px 16px', background: '#ccc', color: '#333', border: 'none', borderRadius: '4px', cursor: 'pointer' }
};